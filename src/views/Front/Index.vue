<template>
  ...
  <div class="top-account d-flex justify-content-end">
      ...
      <ul class="top-account-list d-flex">
        <li :class="{ 'slide-left': isSlide === true }">
          <router-link to="/login" class="px-2">
            ...
            <span class="top-account-slide slide1">{{
              $t("Index.login")
            }}</span>
          </router-link>
        </li>
        <li
          :class="{ 'slide-right': isSlide === true }"
          v-if="cartLength === 0"
        >
          <a href="#" class="px-2 isCart" @click.prevent="EmptyCart">
            ...
            />
            <span
              class="top-account-cart-num badge rounded-pill isCart"
              v-if="cartLength > 0"
            >
    ...
    <!-- 購物車下滑選單 -->
    <CartModal :cart="cart" :cartLength="cartLength" :hide="hide"></CartModal>
  </div>
  <NavBarSm :cartLength="cartLength"></NavBarSm>
  <IndexSwiper></IndexSwiper>
  <div
    class="container main_container"
    :style="{ maxWidth: width + 'px', marginLeft: marginLeft + 'px' }"
  >
    <!-- News -->
            ...
            <ul>
              <li>
                <a href="#/index" class="fw-bold"
                  >{{ $t("Index.important-news-contentFirst")
                  }}<span class="text-danger">{{
                    $t("Index.important-news-contentTime")
                  }}</span
                  >{{ $t("Index.important-news-contentLast") }}
                  <span class="text-decoration-underline underline">{{
                    $t("Index.important-news-contentLast-line")
                  }}</span></a
                >
              </li>
            </ul>
          ...
          <li>
            ...
            >
              <div class="news_list_content">
                ...
                <h3>{{ $t("Index.news-content-1") }}</h3>
              </div>
            </div>
          </li>
          <li>
            <div class="news_list_img2"></div>
            <div
              class="
                news_list_description
                d-flex
            ...
            >
              <div class="news_list_content">
                ...
                <h3>{{ $t("Index.news-content-2") }}</h3>
              </div>
            </div>
          </li>
          <li>
            <div class="news_list_img3"></div>
            <div
              class="
                news_list_description
                d-flex
    ...
    <section class="info" id="info">
      <div class="row">
        ...
        >
          <h3>企業情報</h3>
          <div class="info-content pb-3">
            <p class="pt-4">{{ $t("Index.company-info-top") }}</p>
            <p>{{ $t("Index.company-info-down") }}</p>
          </div>
          <router-link to="/index#info" class="text-primary">{{
            $t("Index.company-info")
          }}</router-link>
        </div>
